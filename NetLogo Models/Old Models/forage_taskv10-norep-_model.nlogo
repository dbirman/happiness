...
foragers-own [ntask taskn memt memv val aim m? s? e? energy age hv smem hed hmem eud emem pref]
...
to pick-next-task
  if not m? and random-float 1 < .25 [
    ifelse hed-eud [
      ...
      let bad-eud not (random-normal 0 es < (eud / es))
      let bad-hed not (random-normal 0 hs < (hed / es))
...
to-report calc-hed
  ...
  let tval 0
  foreach [1 5] [
    set tval tval + ifelse-value (taskn = ?) [ nhfval * helper-task-mean-value ? true ] [ nhfval / 2 * helper-task-mean-value ? true ]
  ]
  report tval
end
...
to-report helper-eud-htmv [n]
  report helper-task-mean-value n true - helper-task-mean-value n false
end
...
true
true
"" "if count foragers > 0 and remainder ticks 2 = 0 [\nlet hedv mean [(calc-hed - mean hmem) / helper-std hmem] of foragers\nlet eudv mean [(calc-eud - mean emem) / helper-std emem] of foragers\nset-current-plot-pen \"hed\"\nplot hedv\nset-current-plot-pen \"eud\"\nplot eudv\n]"
PENS
"hed" 1.0 0 -8990512 true "" ""
"eud" 1.0 0 -13791810 true "" ""
