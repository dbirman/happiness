...
to foragers-do
  ask foragers [
    ...
    let ch calc-hed
    let ce calc-eud
    set hed (ch - helper-mean hmem)
    set eud (ce - helper-mean emem)
    helper-mem ch ce
    pick-next-task
  ]
end
...
to collect
  if [penergy] of patch-here > 0 [
    let ge [penergy] of patch-here * collect-rate * max (list ((mem-long - smem) / mem-long) 0)
...
to pick-next-task
  ifelse hed-eud = 1 [
    ...
    ifelse hedt [
      ifelse eudt [
        helper-st2 one-of [1 2 3 4 4 4]
      ] [
        helper-st2 one-of [1 2 3 3 3 4]
      ]
    ] [
      helper-st2 one-of [1 1 2 2 3 4]
    ]
  ] [
  ;;Not using Hed/Eud
...
to patches-do
  ...
  if count patches with [seed?] < num-patches [ask one-of patches [helper-seedpatch]]
  ask patches [
    if penergy > 0 [set ps ps + log (penergy + 1) 10]
    set ps ps - .05
    set fs fs - .1
    ifelse fs > 0 [set pcolor 52 + fs / mfs * 4] [
      set fs 0
