...
```{r}
ggplot(data,aes(ticks,hed,color=runname)) +
...
```{r}
rs_h = lmer(data=data,hed~hunger+thirst+social+runname+seedrateF+(1|run))
rs_e = lmer(data=data,eud~hunger+thirst+social+runname+seedrateF+(1|run))
summary(rs_h)
summary(rs_e)
```
