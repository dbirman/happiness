...
foragers-own [ntask taskn memt memv val e? energy age hv smem hed hmem eud emem eatmem rmem lco repc]
globals [dead seeded seed-list next-seed mem-long wither-rate control]
...
to pick-next-task
  ...
  ifelse hed-eud = 1 [
    ifelse random-float 1 < pref [
      ...
      ifelse hed [
        ;;HEDONIC HAPPY
        set pick-list [2 5] ;; ONLY WANDER
      ] [ 
        ;;HEDONIC UNHAPPY
        if energy <= rep-energy * h-mult * 10 [set pick-list lput 1 pick-list]
        if rmem > mem-short [set pick-list lput 4 pick-list]
      ]
    ] [
      ;; EUDAIMONIC = MORE THAN
      ifelse eud [ 
        ;;EUD HAPPY
        set pick-list [2 5];; ONLY OBSERVE
      ] [
        ;;EUD UNHAPPY
        if energy <= rep-energy * h-mult * 10 [set pick-list lput 1 pick-list]
        if rmem > mem-short [set pick-list lput 4 pick-list]
        if smem > mem-short [set pick-list lput 3 pick-list]
      ]
    ]
  ] [
  ;;Not using Hed/Eud
    set pick-list [1 2 3 4 5]
  ]
  if pick-list = [] [set pick-list [5]]
  helper-st2 one-of pick-list
end
...
to-report calc-eud
  ...
  [report sum memv] ;;helper-task-sum-value 1 false + helper-task-sum-value 4 false + helper-task-sum-value 3 false]
end
...
to patches-do
  ...
  ask patches [
    ...
    ifelse seed? [
      ...
      if random-float 1 < die-chance [helper-replacepatch]
    ] [
    ;; Wither
    set penergy penergy - const-energ-dec
    if seed-val > 0 [ set seed-val seed-val * (1 - wither-rate) - wither-rate ]
    ]
  ]
  diffuse ps .5
  diffuse fs .5
...
to helper-replacepatch
  set seed? false
  ask one-of patches with [not seed?] [helper-seedpatch]
  set seeded patches with [seed?]
end
...
set error-val .25</setup>
  ...
  <experiment name="E2-Initial_Vals" repetitions="2" runMetricsEveryStep="false">
    <setup>reset
forage</setup>
    <go>go</go>
