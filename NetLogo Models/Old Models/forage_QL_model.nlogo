...
foragers-own [next-task taskn hed eud-val m-mat time-eat time-soc time-rep prev-in prev-sol prev-err age]
...
to foragers-do
  ...
  ask foragers [
    ...
    fhelper-discount v sol ;; change the weights in the m-mat
    fhelper-basics
    run next-task
...
to-report fhelper-values
  ...
  ;;SENSES
  set prev-in (sentence hed (eud-val / 50 - 1) te ts tr prev-sol)
...
to forager-pick-task [sol]
  let bsol boltz sol
  let i position (max bsol) bsol + 1
  fhelper-snt i
end
...
to fhelper-discount [values sol]
  ...
  let Qs'a max sol
  let new-Q (1 - beta) * item i prev-sol + beta * (hed + gamma * Qs'a)
  ;let new-Q hed + gamma * Qs'a
  ;if [who] of self = 681 [show new-Q show Qs'a]
  ...
  foreach n-values length prev-in [?] [
    ...
    set dw ((learn-rate) * err * item i prev-in)
    set c replace-item ? c (item ? c + dw + delta * dw)
  ]
...
to rep
  if random-float 1 < rep-rate and time-eat < mem / 5 and time-soc < mem / 2 [
    set time-eat time-eat + 10
    set time-rep 0
    ;hatch-foragers 1 [
    ;  set color [color] of myself + random-normal 0 .2
    ;  set eud-val 0
...
to fhelper-basics
  ...
  if ticks > 2 [set hed neud - eud-val + ifelse-value (time-rep < 15) [(10 - time-rep) / 5] [-1]]
  set eud-val neud
end
...
to-report mean-m-mat
  report mean-matrix [m-mat] of foragers
end
to-report mean-d-mat
  report [];;report mean-matrix [d-mat] of foragers
end

to-report mean-matrix [m-list]
  ifelse count foragers > 0 [
    let dims matrix:dimensions item 0 m-list
    let nmat item 0 m-list
    foreach n-values item 0 dims [?] [
      let x ?
      foreach n-values item 1 dims [?] [
        let y ?
        let c-list []
        foreach n-values length m-list [?] [
          set c-list lput matrix:get item ? m-list x y c-list
        ]
        matrix:set nmat x y mean c-list
      ]
    ]
    report nmat
  ]
