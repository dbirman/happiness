...
;; Code for running an "example" run
to example
  reset
  local-seed
  forage
...
;; Reset variables, ticks, and fix the globals
to reset
  clear-all
  reset-ticks
  patches-reset
...
;; Run one "tick" of the experiment
to go
  ;;PATCHES
  ;; This special code is designed to ensure that the environment is consistent across runs. It generates a new seed on each run, based on the old seed, ensuring randomness is constant.
  with-local-randomness [
    if ticks = 0 [local-seed]
    cur-seed
    patches-do
  ]
  ;; I don't use local randomness for foragers, so they will differ on every run.
  ;;FORAGERS
  foragers-do
  ;; Make sure to 'tick' at the end, or the code doesn't work properly.
  tick
end
...
;; Add foragers to the environment and give them some energy
to foragers-add
...
;; Do all of the things foragers need to do
to foragers-do
  ask foragers [
    ...
    ;; Now run a task
    run ntask
    ;; Calculate values for eud and hed
    let ce calc-eud
    set hed calc-hed
    set eud ce > eval
    ;; Put the hedonic and eudaimonic values in memory
    helper-mem (ifelse-value hed [1] [0]) ce
    ;; Use these to pick a task
    pick-next-task
  ]
end

;; Helper function to kill foragers
to foragers-die
  if energy <= 0 [
    set dead dead + 1
    die
...
;; Each task can be "run" by a forager to do one activity
;; taskn is the current task a forager is doing
...
to collect
  ...
  ;; If there's no energy here, observe
  set taskn 2
  observe
  ]
end

;; A helper function for collecting, adjusts the collection rate by how recently the forager socialized
to-report calc-soc-multiplier
  report max (list ((mem-long - smem) / mem-long) 0)
end

;; Observes nearby patches with energy and moves there
to observe
...
;; Socializes with foragers that are here, or goes towards patches with foragers
to socialize
...
;; With some random chance, create a new forager on this spot
to reproduce
...
to pick-next-task
  ;; First check if we are using hed and eud values
  ifelse hed-eud = 1 [
    ;; Now check whether we want to check hedonic on this tick, or eudaimonic
    ifelse random-float 1 < pref [
      ;; The rest of this code controls what happens given hedonic happiness, or eudaimonic happiness
...
to patches-do
  ...
  ask patches [
    ...
    ifelse seed? [
      ...
      ;; Wither
      ;; This is just some code that makes patches "fade" away nicely
...
;; A bit complicated, uses two lists memt and memv to find the sublist memv(memt==n), and sums this
...
;; Helper function to deal with memory values (stores hedonic value and eudaimonic values)
...
;; Set-Task-To "n", using the next helper num-to-task
to helper-st2 [n]
...
;; Basic value setup, this isn't strictly necessary but it's good coding practice in netlogo, otherwise you get strange errors
...
;; Patch setup for seeding
to helper-seedpatch
  set seed? true
  set seed-val random-float max-seed-val
end

;; Used for local randomness
to cur-seed
  random-seed next-seed
  set next-seed random 100000
end

;; Moves a forager "uphill" based on the ps values
to helper-uphill-ps
...
;; Looks for patches that are visible within 180 degrees forward, ignoring the patch the forager is on
...
;; These are for dealing with statistics, so that patch values (such as patch-scent), have some variability
to-report fs-error
