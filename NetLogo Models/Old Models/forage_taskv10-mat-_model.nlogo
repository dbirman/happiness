...
foragers-own [ntask taskn nhfval memt memv val aim m? s? e? energy age hv smem hed hmem eud emem htmvs htmvl]
...
to go
  ...
  with-local-randomness [
    if count patches with [seed?] = 0 [patches-seed]
    cur-seed
    patches-grow
  ]
  ;;PENS
...
to foragers-do
  ask foragers [
    foragers-die
    set nhfval NHF
    set htmvs (list helper-task-mean-value 1 true helper-task-mean-value 2 true helper-task-mean-value 3 true helper-task-mean-value 4 true)
    set htmvl (list helper-task-mean-value 1 false helper-task-mean-value 2 false helper-task-mean-value 3 false helper-task-mean-value 4 false)
    set age age + 1
    set val 0
    set s? false
    set energy energy - e-per-tick
    run ntask
    set hed (calc-hed - helper-mean hmem)
    set eud (calc-eud - helper-mean emem)
    helper-mem
    pick-next-task
  ]
end
...
to observe
  ifelse m? [ moving ] [
    ...
    ifelse not (aim = patch-here) [
      set m? true
      set hv obs-mult * ([helper-penergyerror] of aim - [helper-penergyerror] of patch-here) / distance aim / distance aim
      set val hv
    ] [set val 1 * obs-mult]
  ]
end
...
to socialize
  ifelse m? [ moving ] [
    let cfor max-one-of other foragers in-radius view-radius [helper-energyerror]
    ifelse not (cfor = nobody) [
      set aim [patch-here] of cfor
      set m? true
      ifelse not (aim = patch-here) [
      set hv soc-mult * sum smem / distance aim / distance aim
      set val hv
      ] [ set val soc-mult * sum smem ]
    ] [
      set val 0
    ]
  ]
...
to reproduce
  if random-float 1 < (sum smem) / mem-long [ ;; Linear increase in proability of reproducing, only reproduce if you're fully happy
    hatch-foragers 1 [
...
to pick-next-task
  if not m? and random-float 1 < .25 [
    ifelse hed-eud = 1 [
      let es helper-std emem
      let hs helper-std hmem
      let bad-eud not (eud > 0)
      let bad-hed not (hed > 0)
      ifelse bad-eud and bad-hed [
        ;; Hedonically unhappy and eudaimonically unhappy, acts hedonic when hedonic < 1
        helper-st2 one-of ifelse-value (random-float 1 < pref) [[1 2]] [[3 4]]
      ] [ ifelse bad-eud [
        ;; Eudaimonically unhappy only
        helper-st2 one-of [3 4]
      ] [ifelse bad-hed [
      ;; Hedonically unhappy only
      helper-st2 one-of [1 2]
      ] [
      ;; BOTH HAPPY, reproduce
      helper-st2 5
      ]
      ]
      ]
    ] [
      ;;Not using Hedonic/Eudaimonic
...
to-report calc-hed
  let tval 0
  foreach [1] [
    set tval tval + ifelse-value (taskn = ?) [ nhfval * (item (? - 1) htmvs) ] [ nhfval / 2 * (item (? - 1) htmvs) ]
  ]
  report tval
end
...
to-report calc-eud
  let tval 0
  foreach [1 2 3 4] [
    set tval tval + ifelse-value (taskn = ?) [ nhfval * helper-eud-htmv ? ] [ nhfval / 5 * helper-eud-htmv ? ]
  ]
  report tval
end
...
to-report NHF
  let c 0
  while [c < length memt and item c memt = taskn] [set c c + 1]
  let v max-nhf - (c * nhf-dropoff)
  ifelse v > 0 [report v] [report 0]
end
...
to patches-grow
  ask patches [
    ifelse seed? [
      ...
      if seed-val > 0 [
        set seed-val seed-val - (seed-val * wither-rate) - wither-rate
        ifelse seed-val > 0
...
to-report helper-eud-htmv [n]
  report (item (n - 1) htmvs) - (item (n - 1) htmvl)
end
...
forage</setup>
    <go>go</go>
    <exitCondition>ticks &gt; 100 and random 250 &lt; 1</exitCondition>
    ...
    <enumeratedValueSet variable="wan-val">
      <value value="1"/>
    </enumeratedValueSet>
