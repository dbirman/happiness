...
foragers-own [food energy thresh-eat cur-task ctn memory age aim hed life-eneg last-eneg]
links-own [strength]
;;OTHER SETUP
...
to go
  tick
  ;;if ticks = 10000 [ stop ]
  if remainder ticks 2500 = 0 [ re-do-seeds ]
  do-basics
  do-patches
  do-foragers
  do-cleanup
...
to add-foragers
  create-foragers forager-count [
    ...
    forager-basic-setup -1 100 def-hed-value ;;The -1 is for default thresholds
  ]
end
...
to FT-gather
  ...
  [ 
    ...
    ifelse not (fpatch = nobody) [
      set cur-task task FT-walktogather
      set aim fpatch
    ]
    [ FT-new 4 ]
  ]
...
to FT-walktoshare
  ...
  if patch-here = aim [set cur-task task FT-share]
end
...
to FT-walktogather
  ...
  move-to min-one-of neighbors [aim-distance]
  set energy energy - move-cost
  if patch-here = aim [set cur-task task FT-gather]
end
...
to FT-sit
  let hlist [2 3 7]
  let elist [1 3 4 5 6]
  if not on-gather [set elist remove 1 elist]
  if not on-eat [set hlist remove 2 hlist]
  if not on-share [
    set hlist remove 3 hlist
    set elist remove 3 elist
  ]
  if not on-wander [set elist remove 4 elist]
  if not on-walkto [set elist remove 5 elist]
  if not on-social [set elist remove 6 elist]
  if not on-mate [set hlist remove 7 hlist]
  ifelse on-hedeud
  [
    ;; If the # of hedonics is > then "hed", then c-d-h-e will be positive. Since we're doing too many hedonics, we want the probability of doing a eudaimonic to increase, so
    ;; we check if a random # is less than this. The hed-eud-perc works in such a way that it takes .5/h-e-p too many hedonics before you're guaranteed to pick a eudaimonic
    ;; on the next move.
    ifelse random-float 1 < (.5 + calc-diff-hed-eud * hed-eud-perc)
    [ FT-new one-of elist ]
    [ FT-new one-of hlist ]
  ]
  [ FT-new one-of sentence elist hlist ]
end
...
to-report calc-diff-hed-eud
   let hc 0
   ;;let ec 0
   foreach memory [
     ;;if ? = 1 [set ec + 1]
     if ? = 2 [set hc hc + 1]
     ;;if ? = 3 [set ec + 1 set hc + 1]
     if ? = 3 [set hc hc + 1]
     if ? = 7 [set hc hc + 1]
   ]
   report hc - hed
...
to create-new-forager
  hatch-foragers 1 [
    forager-basic-setup mutate-threshold mate-energy-transfer [hed] of myself
...
to forager-basic-setup [te ie nh]
  set food 0
  set energy ie
  set hed nh + (random-float 2) - 1;;TODO: ADD A PROPER MUTATOR
  if hed < 0 [set hed 0]
  if hed > 25 [set hed 25]
...
true
true
"" "if count foragers > 0 [\nset-current-plot-pen \"Average\"\nplot mean [life-eneg / (age + 1)] of foragers\nset-current-plot-pen \"Cur\"\nplot mean [last-eneg] of foragers\n]"
PENS
"Average" 1.0 0 -16777216 true "" ""
"Cur" 1.0 0 -7500403 true "" ""
...
I think that the agents need to be UNAWARE of their internal status. This is important for the simplicity, agents should be driven by very simple powers, for example just their happiness. Right now the agents are driven by all sorts of things, including their ability to "see" other agents--IGNORE THAT! Make the agents blind, deaf, and dumb! 

Instead of having both food and energy, make food a unit, and have it transfer to a certain amount of energy (does this really reduce complexity or just shift it?)
