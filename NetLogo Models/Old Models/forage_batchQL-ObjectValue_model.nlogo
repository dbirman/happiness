...
to reset
  ...
  set table matrix:make-constant 5 (2 ^ 6)  0
end
...
to lforagers-do
  ask foragers [ 
    ...
    fhelper-basics
    run next-task
    let cvals fhelper-values
    fhelper-qtable cvals
  ]
end
...
to foragers-do
  ask foragers [
    ...
    forager-pick-task fhelper-calc-out fhelper-values
    fhelper-basics
    run next-task
  ]
end
...
to-report fhelper-values
  report (list (time-eat < 20) (time-soc < 30) (time-rep < 50) (count other foragers-on patch-here > 0) (hed > 0) (eud-val > 75))
end

to-report fhelper-calc-out [values]
  report fhelper-retrieve-table values 
end

to-report fhelper-retrieve-table [values]
  report matrix:get-column table helper-table-pos values
end

to fhelper-set-table [pos nvals]
  matrix:set-column table pos nvals 
end

to forager-pick-task [out]
  let i position (max out) out + 1
  fhelper-snt i
end

to fhelper-qtable [values]
  ;; Q-Learning, set the current table position's value for this action, to: reward + gamma * MAX(Q of best next action)
  let new-Q hed + gamma * max-future-hed
  
  let curt fhelper-retrieve-table values
  let i taskn - 1
  set curt replace-item i curt (item i curt + learn-rate * new-Q)
  let pos helper-table-pos values
  fhelper-set-table pos curt
end
...
to-report max-future-hed
  ...
  let move-val fhelper-calc-eud (time-eat + 1) (time-soc + 1) (time-rep + 1)
...
to-report helper-table-pos [values]
  let pos 0
  let mult 2 ^ 5
  foreach values [ if ? [set pos pos + mult] 
    set mult mult / 2
  ]
  report pos
end
