...
foragers-own [food energy thresh-eat cur-task ctn memory age aim hed]
links-own [strength]
;;OTHER SETUP
...
to FT-sit
  ...
  ifelse on-hedeud
  [ ;; Find the difference between your memory of hedonic/eudaimonic and current
    let diffv calc-diff-hed-eud
    ifelse diffv > 0
    [
      ;; Too many HEDONICS (hc > hed), so for each point, 5% more likely to go eud
      let perc .5 + diffv * .05
      ifelse random-float 1 < perc 
      [ FT-new one-of elist ]
      [ FT-new one-of hlist ]
    ]
    [ ;; Too many EUDAIMONICS (hc < hed)
      let perc .5 + (- diffv) * .05
      ifelse random-float 1 < perc
      [ FT-new one-of hlist ]
      [ FT-new one-of elist ]
